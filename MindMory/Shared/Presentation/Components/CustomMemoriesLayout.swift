...
struct CustomMemoriesLayout<Content: View, Background: View>: View {
...
private struct TopCurveShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        path.addCurve(
            to: CGPoint(x: rect.width * 0.55, y: rect.height * 0.85),
            control1: CGPoint(x: rect.width * 0.95, y: rect.height * 0.77),
            control2: CGPoint(x: rect.width * 0.8, y: rect.height * 0.95)
        )
